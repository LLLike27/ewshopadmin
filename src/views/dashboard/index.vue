  ...
  <div class="content px-4">
    <div class="mt-4 bg-white">
      <div>
        ...
        >
          <p v-for="item in Systems" :key="item.id">
            {{ item.title }}：<a :href="item.url" target="_blank">{{ item.value }}</a>
          </p>
        </n-card>
      </div>
    </div>
      ...
      >
        <div class="goods">
          ...
          <div>
            <div>缺货商品</div>
            <div>
              <span>{{ goods_info.stock_null }}</span
              >件
            </div>
          </div>
          <div>
            <div>推荐商品</div>
            <div>
              <span>{{ goods_info.recommend_nums }}</span
              >件
            </div>
          </div>
        </div>
      ...
      >
        <div class="goods">
          <div>
            <div>今日访客</div>
            <div>
              <span>{{ users_info.today_nums }}</span
              >人
            </div>
          </div>
          <div>
            <div>昨日访客</div>
            <div>
              <span>{{ users_info.yesterday_nums }}</span
              >人
            </div>
          </div>
          <div>
            <div>累计访客</div>
            <div>
              <span>{{ users_info.total_nums }}</span
              >人
            </div>
          </div>
          <div>
