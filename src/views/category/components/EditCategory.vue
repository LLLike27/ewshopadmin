<template>
  <n-modal
    ...
    @update:show="$emit('showEditModal', false)"
    :mask-closable="true"
  >
    <n-card
      style="width: 600px"
    ...
    >
      ...
      <n-form v-if="showForm" ref="formRef" :model="model" :rules="rules">
        <n-form-item style="width: 80%" path="title">
          ...
          <n-input v-model:value="model.name" type="text" placeholder="请输入" />
        </n-form-item>
        <n-row :gutter="[0, 24]">
          <n-col :span="24">
            <div style="display: flex; justify-content: flex-end">
              <n-button round type="primary" @click="categorySubmit"> 提交 </n-button>
            </div>
          </n-col>
        </n-row>
      </n-form>
...
import { categoryInfo, updateCategory } from "@/api/category";
...
onMounted(() => {
  ...
  if (props.category_id) {
    categoryInfo(props.category_id).then((res) => {
